...
class Config:
    def __init__(self, sentinel_data_path=None, network_id=None, bootnode_url=None,
        ...
        if sentinel_data_path is None or sentinel_data_path == 'None':
            self.SENTINEL_DATA_PATH = os.path.join(
                os.environ['HOME'], '.ethereum/sentinel')
        if network_id is None or network_id == 'None':
            self.NETWORK_ID = '78432893'
        if bootnode_url is None or bootnode_url == 'None':
            self.BOOTNODE_URL = ','.join(BOOTNODE_URLS)
        if rpc_apis is None or rpc_apis == 'None':
            self.RPC_APIS = 'admin,debug,eth,miner,net,personal,rpc,txpool,web3'
        if genesis_file is None or genesis_file == 'None':
            self.GENESIS_FILE = os.path.join(
                os.path.dirname(__file__), 'genesis.json')
