...
const appStatsRoute = '/api/client/app/stats'
  ...
  describe('/POST ' + vpnRoute, () => {
    ...
    it('With correct details should give credentials', (done) => {
      chai.request(server)
        ...
        .send(vpnDetails)
        .end((err, res) => {
          console.log('res, err', res.body, err)
          res.should.have.status(200);
          res.body.should.be.a('object');
          done();
        });
...
describe('routes for app stats', () => {
  describe('/POST ' + appStatsRoute, () => {
    it('should return stats on success', (done) => {
      chai.request(server)
        .get(appStatsRoute)
        .end((err, res) => {
          res.should.have.status(200);
