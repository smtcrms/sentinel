...
function windowManager() {
  ...
  this.createWindow = () => {
    ...
    this.window = new BrowserWindow({ title: "Sentinel Network - dVPN - alpha-0.11.2", resizable: false, maximizable: false, width: 1000, height: screenHeight, icon: './public/icon256x256.png' });
...
function stopVPN(cb) {
  if (process.platform === 'win32') {
    try {
      var cmd;
      if (vpnType === 'socks5')
        cmd = 'net stop sentinelSocksv112 /f  && taskkill /IM sentinel.exe /f'
